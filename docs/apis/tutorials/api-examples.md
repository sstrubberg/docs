...
This query follows the old CMS upload process by downloading a file as an asset to an existing TDO that is empty -- see "Create Empty TDO (No Asset)" query in the "Ingestion" section -- and then running one or more engines against that asset.  This works with all V1F (Iron) engines.
...
NOTE: *This call will fail if the caller does not have superadmin rights.*
```graphql
mutation  {
...
### Whitelist All Ingestion Adapters
```graphql
